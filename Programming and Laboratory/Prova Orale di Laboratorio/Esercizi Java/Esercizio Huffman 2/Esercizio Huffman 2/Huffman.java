...
public class Huffman {
  ...
  public static int[] charHistogram( String src ) {
    ...
    return freq; //quante volte compare ciascun carattere
  }
  ...
  //VERSIONE ricorsiva
  public static Node restoreTreeR(InputTextFile in) //funzione per decomprimere l'albero
  {
     char c = in.readChar(); //parte dal nodo foglia

     if( c == '@' ) //se e' @ e' un nodo intermedio
     {
         Node lft = restoreTreeR(in);
         Node rgt = restoreTreeR(in);

         return new Node(lft,rgt);
     }
     else
     {
         if(c == '\\') //se il carattere trovato e' \ (noi scriviamo \\ perche solo uno non e' letto come un char)
         {
             c=in.readChar(); //se c'e' il backslash allora vuol dire che il carattere successivo e' quello che mi serve e sara' o @ o \
         }
         return new Node(c,0);
     }
  }
  ...
  private static char decodeNextChar( Node root, InputTextFile in ) {
    ...
    do { //legge la mossa successiva da fare, 0 vai a sx o 1 vai a dx
      if ( in.readBit() == 0 ) {
