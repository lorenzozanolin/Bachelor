  ...
  private static int llisMemRec( int[] s, int[][] v, int i, int j ){
    ...
    if( v[i][j] == UNKNOWN){// se v[i][j] non e' ancora stato calcolato lo calcola normalmente e lo registra 
      if( i == l )
        ...
        v[i][j] = llisMemRec( s, v, i+1, j );
      else
        v[i][j] = Math.max( 1+llisMemRec( s, v, i+1, i ), llisMemRec( s, v, i+1, j ) );
    }
    return v[i][j];
  }
  ...
  private static IntSList lisRec( int[] s, IntSList[][] m, int i, int j ){
    ...
    if( m[i][j] == NULL_INTLIST){// se v[i][j] non e' ancora stato calcolato lo calcola normalmente 
      if( i == l )
        ...
        IntSList sl1 = lisRec( s, m, i+1, i ).cons( s[i] ); //provo a calcolare prima quella dove aggiungo un nuovo elemento (che diventa massimo)
        IntSList sl2 = lisRec( s, m, i+1, j ); //altrimenti provo a calcolarla proseguendo e ignorando l'elemento corrente
        if( sl1.length() < sl2.length() )//calcolo della lista con piu' elementi confrontando quelle ottenute precedentemente -> mi salvo quella piu lunga
          m[i][j] = sl2;
